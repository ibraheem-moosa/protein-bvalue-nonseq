...
def get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, indices, ws, lr, dt, oh, imp, use_metadata=False):
    pccs = []
    mses = []
    for i in indices:
        X = []
        ...
        X = imp.transform(X)
        X_oh = oh.transform(X)
        y_pred = lr.predict(X_oh) + dt.predict(X)
...
if __name__ == '__main__':
    ...
    print(fi)
    train_pccs, train_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, train_indices, ws, lr, dt, oh, imp, use_metadata)
    val_pccs, val_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, val_indices, ws, lr, dt, oh, imp, use_metadata)
    test_pccs, test_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, test_indices, ws, lr, dt, oh, imp, use_metadata)
