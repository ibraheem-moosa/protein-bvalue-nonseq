...
def get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, indices, ws, lr, rf, oh, imp, use_metadata=False):
    pccs = []
    mses = []
    preds = []
    for i in indices:
        ...
        X = imp.transform(X)
        X_oh = oh.transform(X)
        y_pred = lr.predict(X_oh) + rf.predict(X)
...
if __name__ == '__main__':
    ...
    rf = RandomForestRegressor(max_depth=10, random_state=42, n_estimators=50)
    rf.fit(X, y_delta)
    ...
    print(fi)
    train_pccs, train_mses, train_preds = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, train_indices, ws, lr, rf, oh, imp, use_metadata)
    val_pccs, val_mses, val_preds = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, val_indices, ws, lr, rf, oh, imp, use_metadata)
    test_pccs, test_mses, test_preds = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, test_indices, ws, lr, rf, oh, imp, use_metadata)
