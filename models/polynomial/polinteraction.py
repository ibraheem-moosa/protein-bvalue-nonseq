...
def get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, indices, ws, clf, oh, imp, pol_feature, var_thresh, use_metadata=False):
    pccs = []
    mses = []
    for i in indices:
        X = []
...
if __name__ == '__main__':
    ...
    pol_feature = PolynomialFeatures(interaction_only=True, include_bias=False)
    pol_feature.fit(X)
    X = pol_feature.transform(X)
    print(X.shape)
    var_thresh = VarianceThreshold(1e-10)
    var_thresh.fit(X)
    X = var_thresh.transform(X)
    print(X.shape)
    clf = SGDRegressor(verbose=1, max_iter=1000)
    clf.fit(X, y)
    print("Model fit done.")
    print(clf.score(X, y))
    exit()
    train_pccs, train_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, train_indices, ws, clf, oh, imp, pol_feature, var_thresh, use_metadata)
    val_pccs, val_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, val_indices, ws, clf, oh, imp, pol_feature, var_thresh, use_metadata)
    test_pccs, test_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, test_indices, ws, clf, oh, imp, pol_feature, var_thresh, use_metadata)
