...
def get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, indices, ws, nn, gbt, oh, imp, use_metadata=False):
    pccs = []
    mses = []
    for i in indices:
        X = []
        ...
        X = imp.transform(X)
        X_oh = oh.transform(X)
        y_pred = nn.predict(X_oh) + gbt.predict(X)
...
if __name__ == '__main__':
    ...
    nn = MLPRegressor(verbose=True, solver='adam', learning_rate='adaptive', learning_rate_init=0.0001,
            alpha=0.1, hidden_layer_sizes=hidden_layer_sizes, early_stopping=True, max_iter=100, batch_size=512)
    nn.fit(X_oh, y)
    print("NN fit done.")
    y_delta = y - nn.predict(X_oh)
    gbt = GradientBoostingRegressor(max_depth=3, random_state=42, n_iter_no_change=50, validation_fraction=0.2, n_estimators=250, verbose=1)
    gbt.fit(X, y_delta)
    ...
    print(fi)
    train_pccs, train_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, train_indices, ws, nn, gbt, oh, imp, use_metadata)
    val_pccs, val_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, val_indices, ws, nn, gbt, oh, imp, use_metadata)
    test_pccs, test_mses = get_pccs_and_mses(protein_seqs, protein_bvals, protein_mdatas, test_indices, ws, nn, gbt, oh, imp, use_metadata)
