...
if __name__ == '__main__':
    ...
    clf = MLPRegressor(verbose=True, solver='adam', learning_rate='adaptive', learning_rate_init=1e-3,
            alpha=0.1, hidden_layer_sizes=hidden_layer_sizes, early_stopping=True, max_iter=250, batch_size=512)
    #clf = GridSearchCV(clf, {'learning_rate_init': 1e-3 * 2.0 ** np.arange(-3, 4)}, scoring='neg_mean_squared_error',
    #        cv = ShuffleSplit(n_splits=1, test_size=0.2, random_state=42), verbose=5, n_jobs=-1)
    clf.fit(X, y)
    print("Model fit done.")
    #print("Best LR: {}".format(clf.best_params_['learning_rate_init']))
